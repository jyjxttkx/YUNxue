(function(define)
	...
	local WHITELIST_URL = "https://raw.githubusercontent.com/jyjxttkx/YUNxue/refs/heads/main/Whitelist.lua" 
	...
	if success then
		...
		-- 在获取到的白名单文本前后也加上换行符，以匹配第一行和最后一行
		local fullWhitelistText = "\n" .. whitelistContent .. "\n"

		if string.find(fullWhitelistText, pattern, 1, true) then
			-- 验证通过
			print("Welcome, " .. LocalPlayer.Name .. ". You are whitelisted (Online Check).")
			...
			function ifError(msg)
				warn("脚本出问题辣!")
				writefile(string.format("镜花水月错误日志%s.txt", os.date():sub(11):gsub(" ", "-")), string.format("具体错误原因为:\n %s", msg))
			end
			...
			-- 原本的白名单和开发者检查部分已被新的在线系统替代，故移除
			
			local plr = game:GetService("Players").LocalPlayer;
			game.StarterGui:SetCore('SendNotification', {
				Title = '白名单认证成功',  --> 单引号里面的中文可以改, 加载脚本时的通知
				Text = '玩家 : ' .. plr.Name .. ' 欢迎使用，脚本正在加载...'
			})
			local _warn = warn;
			...
			function library:CreateTab(dZ)
				...
				function e7:NewBox(e9, du, ea)
					...
					--[===[ FIX: Chat Menu Bug Fix ]===]
					-- Original code only updated on Enter. This updates when focus is lost (e.g., clicking a button) OR when Enter is pressed.
					eH.FocusLost:Connect(function(EnterPressed)
						ea(eH.Text) -- Update the variable regardless of how focus was lost
			...
			local bai = {
				...
				sayfast = false,
				-- 传送物品
				cswjia = nil, -- 传送物品的目标玩家
				itemset = nil, -- 物品传送的目标点
				cskais = false, -- 是否正在传送
				dxmz = "", -- 整理或传送时选择的物品名称
				-- 整理物品
				zlwjia = "",
			...
			local function notify(title, text, duration)
				library:Notify({ Title = title, Text = text, Duration = duration or 4 })
			end
						...
						for Z = 1, ZAxis do
							...
							if Items[Count] then
								...
								for e = 1, 40 do
									...
									Items[Count].Main.CFrame = CFrame.new(X * Items[1].Main.Size.X, Y * Items[1].Main.Size.Y, Z * Items[1].Main.Size.Z) + PlayerPos
									game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(Items[Count])
			...
			function sellwood()
				...
				local sellCFrame = CFrame.new(315.12146, - 0.190167814, 85.0448074) -- 商店位置
			...
			function PlankToBlueprint()
				notify('白', '请先点击木板,再点击蓝图', 5)
				local plankModel, blueprintModel
				local connection
				connection = mouse.Button1Down:connect(function()
					local target = mouse.Target.Parent
					if not plankModel and target.Name == "Plank" and target:FindFirstChild("Owner").Value == lp then
						plankModel = target
						notify('白', '已选择木板,请选择蓝图', 4)
					elseif plankModel and not blueprintModel and target:FindFirstChild("Type") and target.Type.Value == "Blueprint" then
						blueprintModel = target
						
						tp(CFrame.new(plankModel:FindFirstChildOfClass('Part').CFrame.p + Vector3.new(0, 3, 4)))
						wait(.2)
						for i = 1, 30 do
							HONG:DragModel(plankModel, blueprintModel.Main.CFrame)
							task.wait(0.01)
						end
						notify('白', '完成', 2);
			...
			function droptool(Position)
				local aQ = lp.Character;
				if aQ:FindFirstChildOfClass "Tool" then
					game.ReplicatedStorage.Interaction.ClientInteracted:FireServer(aQ:FindFirstChildOfClass "Tool", "Drop tool", Position)
				end
				for a, b in pairs(lp.Backpack:GetChildren()) do
					if b.Name == "Tool" and b.ClassName == "Tool" then
						game.ReplicatedStorage.Interaction.ClientInteracted:FireServer(b, "Drop tool", Position)
					end
				end
			end
			...
			function farAxeEquip()
				...
				bai.farAxeEquip = mouse.Button1Down:connect(function()
					...
					if target and target.Parent and target.Parent:IsA('Model') and target.Parent:FindFirstChild('ToolName') then
			...
			function carTeleport(cframe)
				if lp.Character and lp.Character.Humanoid.SeatPart then
		...
		Page1:NewButton("脚本作者  林北#0296", function()
			print("感谢 林北")
		end)
		...
		Page4:NewButton("卖掉所有木板", function()
			local sellCFrame = CFrame.new(315.12146, -0.190167814, 85.0448074)
			local planks = getPlanks()
			for _, plank in pairs(planks) do
				for i = 1, 20 do
					HONG:DragModel(plank, sellCFrame)
					task.wait(0.05)
				end
			end
		end)
		
		Page4:NewButton("卖掉所有原木", function()
			sellwood()
		end)
		
		Page4:NewToggle("自动扔斧头", 'autoDropAxeToggle', false, function(state)
			bai.autodropae = state
			if state then
				while bai.autodropae == true do
		...
		Page4:NewToggle("自动捡斧头", 'autoPickAxeToggle', false, function(state)
			bai.autopick = state
			if state then
				while bai.autopick == true do
					task.wait(0.1)
					for a, b in pairs(workspace.PlayerModels:GetChildren()) do
						if b:FindFirstChild("Owner") and b.Owner.Value == lp and b:FindFirstChild("Type") and b.Type.Value == "Tool" then
		...
		Page4:NewButton("远程装备斧头", function()
			farAxeEquip()
		end)
		--[===[ 新增功能结束 (木材菜单) ]===]
		...
		Page5:NewDropdown("汽车传送", 'carTeleportDropdown', {'出生点', '木材反斗城'}, function(b)
			if b == '木材反斗城' then
				carTeleport(CFrame.new(270, 4, 60));
			elseif b == '出生点' then
				carTeleport(CFrame.new(155, 3, 74))
			end
		end)
	
		Page5:NewDropdown("传送到树", 'treeTeleportDropdown', {"普通树", "幻影木"}, function(b)
			local tree_name = ""
			if b == "普通树" then tree_name = "Generic" end
			if b == "幻影木" then tree_name = "LoneCave" end
			
			for i, v in pairs(game.Workspace:GetChildren()) do
				if v.Name == "TreeRegion" then
					for j, k in ipairs(v:GetChildren()) do
						if k:FindFirstChild("TreeClass") and k.TreeClass.Value == tree_name then
							lp.Character:MoveTo(k.WoodSection.Position)
							return
						end
					end
				end
			end
			notify("错误", "未找到该类型的树", 4)
		end)
		--[===[ 新增功能结束 (传送菜单) ]===]
		...
		-- The original cooperAxeDupe function has been replaced with the new code.
		function cooperAxeDupe()
			local Players = game:GetService("Players")
			local player = Players.LocalPlayer

			local TARGET_X, TARGET_Y, TARGET_Z = 283, -354, 47.5
			local Y_OFFSET = 4 
			...
			local function teleportOnce()
				...
				local targetPos = Vector3.new(TARGET_X, TARGET_Y, TARGET_Z) + Vector3.new(0, Y_OFFSET, 0)
			...
			local LSR = ReplicatedStorage:FindFirstChild("LoadSaveRequests")
			if not LSR or not LSR:FindFirstChild("RequestLoad") then
				warn("[TurboReload] 未找到 LoadSaveRequests/RequestLoad")
				return
			end

			local g = (getgenv and getgenv()) or {}
			local specified = tonumber(g.SLOT_TO_LOAD or g.SLOT)

			local function valid(n) return type(n)=="number" and n>=1 and n<=6 and n%1==0 end

			local function tryRequestLoad(slot)

				local ok,res = pcall(function() return LSR.RequestLoad:InvokeServer(slot) end)
				if ok and res == true then return true end

				ok,res = pcall(function() return LSR.RequestLoad:InvokeServer(slot, true) end)
				if ok and res == true then return true end

				ok,res = pcall(function() return LSR.RequestLoad:InvokeServer(slot, false) end)
				if ok and res == true then return true end
			...
			local target
			if valid(specified) then
				target = specified
			else
				local curObj = LocalPlayer:FindFirstChild("CurrentSaveSlot")
				if curObj and valid(curObj.Value) then
					target = curObj.Value
				end
			end
			...
			local function go(slot)
				...
				local ok = tryRequestLoad(slot)
				print(("[TurboReload] 尝试槽位 %d -> %s"):format(slot, ok and "成功" or "失败"))
				return ok
			end
		...
		--[===[ FIX: Clarified Button Label ]===]
		Page8:NewButton("开始刷粉车", function() 
			_CONFIGS["刷粉车"] = true
			local UserInputService = game:GetService("UserInputService")
		...
		local Gyro, Velocity, carFlyConnection
		
		Page8:NewSlider('汽车飞行速度', 'carFlySpeedSlider', 50, 10, 500, false, function(s)
			carFlySpeed = s
			if Velocity then Velocity.Velocity = HONG.WKSPC.CurrentCamera.CFrame.LookVector * carFlySpeed end
		end)
		
		--[===[ FIX: Car Fly Toggle Functionality ]===]
		Page8:NewToggle("汽车飞行", 'carFlyToggle', false, function(state)
			if state then
				if not (lp.Character and lp.Character.Humanoid and lp.Character.Humanoid.SeatPart) then
					notify("错误", "请先上车再开启飞行", 3)
					library:UpdateToggle("carFlyToggle", false) -- Turn off the toggle in UI
					return
				end
	
				local Car = lp.Character.Humanoid.SeatPart.Parent
				Gyro = Instance.new("BodyGyro", Car.PrimaryPart)
				Gyro.MaxTorque = Vector3.new(math.huge, math.huge, math.huge)
				Velocity = Instance.new("BodyVelocity", Car.PrimaryPart)
				Velocity.MaxForce = Vector3.new(math.huge, math.huge, math.huge)
				Velocity.Velocity = Vector3.new(0,0,0)
	
				carFlyConnection = game:GetService("RunService").RenderStepped:Connect(function()
					if not (Gyro and Gyro.Parent and Velocity and Velocity.Parent and lp.Character and lp.Character.Humanoid.SeatPart) then
						library:UpdateToggle("carFlyToggle", false)
						return
					end
	
					Gyro.CFrame = HONG.WKSPC.CurrentCamera.CFrame
					if lp.Character.Humanoid.MoveDirection.Magnitude > 0 then
						Velocity.Velocity = HONG.WKSPC.CurrentCamera.CFrame.LookVector * carFlySpeed
					else
						Velocity.Velocity = Vector3.new(0,0,0)
					end
				end)
		...
		local vnoclipParts = {}
		Page8:NewToggle("汽车穿墙", 'carNoclipToggle', false, function(state)
			if state then
				if lp.Character and lp.Character.Humanoid.SeatPart then
					local vehicleModel = lp.Character.Humanoid.SeatPart.Parent
					...
					notify("错误", "请先上车", 3)
					library:UpdateToggle("carNoclipToggle", false) -- Turn off toggle
				end
			else
				for part, _ in pairs(vnoclipParts) do
		...
		Page8:NewSlider('汽车默认速度', 'carDefaultSpeedSlider', 50, 3, 600, false, function(s)
			local speed = s
			if lp.Character and lp.Character.Humanoid.SeatPart then
				local car = lp.Character.Humanoid.SeatPart.Parent
				if car and car:FindFirstChild("Configuration") and car.Configuration:FindFirstChild("MaxSpeed") then
					car.Configuration.MaxSpeed.Value = speed
				end
			end
		end)
		--[===[ 新增/修复功能结束 (汽车菜单) ]===]
		...
		--[===[ 新增UI页面开始 (已修复) ]===]
		local Page9 = library:CreateTab("聊天菜单")
		Page9:NewBox("你要说的话", "输入后按回车", function(txt)
			bai.saymege = txt
		end)
		Page9:NewBox("说话次数", "输入数字后按回车", function(txt)
			bai.saymount = tonumber(txt) or 1
		end)
		Page9:NewButton("说话", function()
			bai.sayfast = true
			for i = 1, bai.saymount do
				if bai.sayfast == true and bai.saymege ~= "" then
					game:GetService('ReplicatedStorage').DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bai.saymege, 'All')
					task.wait(0.1) -- Prevent freezing
				else
					bai.sayfast = false
					break
				end
		...
		Page9:NewButton("停止说话", function()
			bai.sayfast = false
		end)
		Page9:NewToggle("全自动说话", 'autosayToggle', false, function(state)
			bai.autosay = state
			spawn(function()
				while bai.autosay and task.wait(1) do
					if bai.saymege ~= "" then
						game:GetService('ReplicatedStorage').DefaultChatSystemChatEvents.SayMessageRequest:FireServer(bai.saymege, 'All')
					end
				end
		...
		local Page10 = library:CreateTab("传送物品")
		local PlayersDropdown = Page10:NewDropdown("选择玩家", 'playerDropdownTransmit', bai.dropdown, function(v)
			bai.cswjia = v
		end)
		Page10:NewButton("刷新玩家列表", function()
			shuaxinlb(true)
			PlayersDropdown:SetOptions(bai.dropdown)
		end)
		Page10:NewButton("设置传送点", function()
			pcall(function() game.Workspace.baiBasedropCord:Destroy() end)
			bai.itemset = lp.Character.HumanoidRootPart.CFrame
			local part = Instance.new("Part", workspace)
			part.Name = "baiBasedropCord"
			part.CFrame = bai.itemset
			part.Anchored = true
			part.CanCollide = false
			part.Size = Vector3.new(4,1,4)
			part.BrickColor = BrickColor.new("Bright green")
		end)
		Page10:NewButton("删除传送点", function()
			pcall(function() game.Workspace.baiBasedropCord:Destroy() end)
			bai.itemset = nil
		end)
		Page10:NewButton("获得传送工具", function()
			if bai.itemset == nil then return notify("白", "请你先设置传送点", 4) end
			if lp.Backpack:FindFirstChild("点击你想要传送的物品") then lp.Backpack["点击你想要传送的物品"]:Destroy() end
			...
			Tool.Activated:connect(function()
				...
				if target:FindFirstChild("Owner") and (target.Owner.Value == lp or target.Owner.Value == nil) then
					if (lp.Character.HumanoidRootPart.CFrame.p - target.PrimaryPart.CFrame.p).magnitude > 10 then
						tp(target.PrimaryPart.CFrame + Vector3.new(4, 0, 4))
					end
					for e = 1, 20 do
						HONG:DragModel(target, bai.itemset)
						game:GetService('RunService').Stepped:wait();
					end
				else
					notify("错误", "你无法移动这个物品", 4)
				end
			end)
		end)
	
		local Page11 = library:CreateTab("整理菜单")
		Page11:NewBox("x轴", "3", function(txt) bai.zix = tonumber(txt) or 3 end)
		Page11:NewBox("z轴", "3", function(txt) bai.zlz = tonumber(txt) or 3 end)
		Page11:NewButton("获取整理工具", function()
			if lp.Backpack:FindFirstChild("点击要整理的物品") then lp.Backpack["点击要整理的物品"]:Destroy() end
			...
			Identify.Activated:connect(function()
				...
				if item and item:FindFirstChild("PurchasedBoxItemName") then
					local itemName = item.PurchasedBoxItemName.Value
					bai.zlwjia = lp.Name
					ItemStacker(itemName, bai.zix, bai.zlz)
				else
					notify("错误", "无法识别此物品", 4)
				end
			end)
			Identify.Parent = lp.Backpack
		end)
		Page11:NewSeparator()
		Page11:NewDropdown("选择要整理的木板类型", "plankTypeDropdown", {"全部"}, function(v) bai.zlmt = v end)
		Page11:NewToggle("竖着放", "verticalPlankToggle", false, function(state) bai.shuzhe = state end)
		Page11:NewButton("开始整理木板", function()
			if bai.zlmt == nil then return notify("白", "你没有选择木头", 4) end
			...
			for _, Plank in pairs(planks) do
				...
				for i = 1, 50 do
					...
					if bai.shuzhe then
						 Plank:SetPrimaryPartCFrame(CFrame.new(oldpos) * CFrame.Angles(math.rad(90), 0, 0))
					else
						Plank.PrimaryPart.Position = oldpos
					end
					game:GetService("ReplicatedStorage").Interaction.ClientIsDragging:FireServer(Plank)
					game:GetService('RunService').Stepped:wait();
				end
		...
		local Page12 = library:CreateTab("金币菜单")
		local donateDropdown = Page12:NewDropdown("选择玩家名字", 'playerDropdownDonate', bai.dropdown, function(v)
			bai.moneytoplayername = v
		end)
		Page12:NewBox("给玩家转钱数量", "输入数量后按回车", function(txt)
			bai.moneyaoumt = txt
		end)
		Page12:NewButton("开始转钱", function()
			donate(bai.moneytoplayername, bai.moneyaoumt)
		end)
		Page12:NewButton("刷新玩家", function()
			shuaxinlb(true)
			donateDropdown:SetOptions(bai.dropdown)
		end)
		...
		--[===[ 新增UI页面结束 ]===]
			...
			-- 验证失败，不在名单内
			LocalPlayer:Kick(LocalPlayer.Name .. "，你没有被列入白名单，请联系管理员购买或反馈。")
		end
	else
		-- 获取白名单失败 (网络问题或链接错误)
		warn("无法获取在线白名单: " .. tostring(whitelistContent))
		LocalPlayer:Kick("白名单服务器验证失败，请检查网络或联系管理员。")
	end

end)("镜花水月伐木大亨2")  --> 脚本名字, 双引号里面的中文可以改
...
-- 获取目标TextLabel
local screenGui = CoreGui:FindFirstChild("镜花水月")
if not screenGui then return end

-- 主窗口Frame下的标题TextLabel
